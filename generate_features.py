...
def generate_feature_sets_by_one_file(path_to_file, to_save, sensors, columns=None, len_sub_window=0, fs=256,
    ...
    # sl_windows = [pd.read_csv('muse2_file/rolling_windows/' + str(el)) for el in os.listdir('muse2_file/rolling_windows')]

    for index in range(len(datasets)):
        # Sliding windows start & end to datetime
        # sl_windows[index]['start'] = [datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f').time() for x in sl_windows[index]['start']]
        # sl_windows[index]['end'] = [datetime.strptime(x, '%Y-%m-%d %H:%M:%S.%f').time() for x in sl_windows[index]['end']]
